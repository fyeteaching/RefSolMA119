...
9. As the variable appears in the denominators, and fractions undefined if the denominator is zero, the restrictions on $x$ is $x \neq 0$ **AND** $x^2 + 3x - 10 \neq 0$. Since $x^2 + 3 x - 10 = (x - 2)(x + 5)$. The restriction is equivalent to $x\neq 2$ and $x\neq -5$. Given the restriction, one way to solve the rational equation is to first clearing the denominator by multiplying the LCD. Due to the restrictions, after solving the resulting equation, the solutions must be checked. In this case, the LCD is $x^2 + 3 x - 10 = (x - 2)(x + 5)$.
    $$
    \begin{aligned}
    \frac{2}{x - 2}-\frac{14}{x^2 + 3x - 10}=&1\\
    (x - 2)(x + 5)\cdot \frac{2}{x - 2} - (x^2 + 3 x - 10)\cdot\frac{14}{x^2 + 3x - 10} = & x^2 + 3 x - 10\\
    2(x + 5) - 14 =& x^2 + 3 x - 10\\
    2x - 4 = & x^2 + 3 x - 10\\
    x^2 + x - 6 =&0\\
    (x - 2)(x + 3)=&0\\
    ...
    \end{aligned}
    $$
    Since the LCD is zero for $x=2$ and nonzero for $x = -3$, only $x = -3$ is a solution.
...
13.  Solving a radical equation is similar to solving an absolute value equation or an exponential equation. A radical expression should be isolated first. Then a functional transformation, taking power of both sides, can be applied. But be careful, not all functional transformations are equivalent transformations.
    $$
    \begin{aligned}
    \sqrt{2 x - 1} + 2 = & x\\
    \sqrt{2 x - 1} = & x - 2\\
    (\sqrt{2 x - 1})^2 = & (x - 2)^2\\
    2 x - 1 = & x^2 -2\cdot 2\cdot x + 2^2\\
    x^2 - 6x + 5 = & 0\\
    (x - 1)(x - 5)=&0\\
    ...
    \end{aligned}
    $$
    **Check:** when $x=2$, the left-hand side is $\sqrt{2 \cdot 1  - 1 } + 2= 1 + 2 = 3$ while the right-hand side is 1. So $x = 1$ is not a solution.
    When $x = 5$, the left-hand side is $\sqrt{2 \cdot 5 - 1} + 2 = \sqrt{9} + 2 = 3 + 2 = 5$ which equals the right-hand side x=5. So $x=5$ is a solution. 
